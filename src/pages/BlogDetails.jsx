...
import BlogLikeContainer from "../components/BlogLikeContainer";
...
const BlogDetais = () => {
    ...
    const { getRequest, data: blog, loading, setLoading } = useGetRequest()

    const [userInfo, setUserInfo] = useState(null)
    const [modal, setModal] = useState(false) 

    const match = blog?.author === user?.username

    useEffect(() => {
        const fetchBlog = async () => {
            await getRequest('/api/blogs/' + id)
        }
        user && fetchBlog()
    }, [user])
    ...
    return ( 
        <div className="BlogDetails">
            ...
            {blog && <>
                ...
                {userInfo && <BlogLikeContainer blog={blog} userInfo={userInfo}/>}
                
                {match && <div className="button-box">
                <button 
                ...
                <h3 className="comment-section__title">Comment Section</h3>
                <CommentContextProvider>
                    <CommentSection blog={blog} userInfo={userInfo}/>
                </CommentContextProvider>
            </>  }
            {modal && <DelBlogModal setModal={setModal} id={id}/>}
        </div>
     );
}
